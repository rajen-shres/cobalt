...
def _get_player_names_by_id(usebio):
    """Helper to get the player names"""

    # First get all the players names
    player_dict = {}
    for item in usebio["PARTICIPANTS"]["PAIR"]:
        ...
        player_dict[this_pair_id] = players_names
...
def _insert_card_symbol(card_string):
    """helper to take 2H and return 2&heart; etc"""

    card_string = card_string.replace("H", "♥")
    card_string = card_string.replace("S", "♠")
    card_string = card_string.replace("D", "&diamond;")
    card_string = card_string.replace("C", "&clubs;")
    ...
    for board in usebio["BOARD"]:
        ...
        for traveller_line in board.get("TRAVELLER_LINE"):
            ...
            if pair_id in [ns_pair, ew_pair]:
                ...
                if pair_id == ns_pair:
                    opponents = player_dict.get(ew_pair)
                    ns_flag = True
                else:
                    opponents = player_dict.get(ns_pair)
                    ns_flag = False
                contract = _insert_card_symbol(traveller_line.get("CONTRACT"))
                played_by = traveller_line.get("PLAYED_BY")
                lead = _insert_card_symbol(traveller_line.get("LEAD"))
...
def usebio_mp_pairs_board_view(request, results_file_id, board_number, pair_id):
    ...
    for board in usebio["EVENT"]["BOARD"]:
        ...
        if this_board_number == board_number:
            ...
            for traveller_line in board.get("TRAVELLER_LINE"):
                ns_pair = player_dict.get(traveller_line.get("NS_PAIR_NUMBER"))
                ew_pair = player_dict.get(traveller_line.get("EW_PAIR_NUMBER"))
